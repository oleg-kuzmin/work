...
//* квантификация
{} - квантификация (сколько символ должен повторятся)
a{4} - выражение из 4 символов a подряд
a{4,6} - выражение от 4 символов до 6 подряд
символ * - выражение повторяется от 0 и выше
ab* - ищет выражение a и b сколько угодно раз
символ + - выражение повторяется от 1 и выше
ab+ - ищет выражение a и b как мининум один раз
символ ? - выражение повторяется либо 0 либо 1
...
// Группы — последовательности символов, заключённые в скобки (), которые могут повторяться с помощью квантификаторов. Группы также позволяют использовать обратные ссылки, которые ссылаются на результат совпадения группы в регулярном выражении.
...
const regex10 = /\d/g; // экранируем метасимвол "\d", который обозначает цифру
...
const regex11 = /\s/g; // экранируем метасимвол "\s", который обозначает пробел
...
const regex12 = /(?<!\d)\.|\.(?!\d)/g; // экранируем точку, используем негативный и позитивный просмотр вперёд и назад для исключения точек, находящихся внутри чисел
