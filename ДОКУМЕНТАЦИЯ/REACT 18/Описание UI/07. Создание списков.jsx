...
// Представьте, что файлы на вашем рабочем столе не имеют имен. Вместо этого вы бы обращались к ним по порядку — первый файл, второй файл и так далее. Вы могли бы привыкнуть к этому, но как только вы удалите файл, это сбивает с толку. Второй файл станет первым файлом, третий файл станет вторым файлом и так далее.

// Имена файлов в папке и ключи JSX в массиве служат той же цели. Они позволяют нам однозначно идентифицировать элемент среди его братьев и сестер. Хорошо подобранный ключ предоставляет больше информации, чем позиция в массиве. Даже если позиция изменится из-за переупорядочивания, key React позволяет идентифицировать элемент на протяжении всего его жизненного цикла.

//* Примечание
// У вас может возникнуть соблазн использовать индекс элемента в массиве в качестве его ключа. На самом деле это то, что будет использовать React, если вы вообще не укажете key. Но порядок, в котором вы отображаете элементы, со временем изменится, если элемент будет вставлен, удален или если массив будет переупорядочен. Индекс как ключ часто приводит к незаметным и запутанным ошибкам.

// Точно так же не генерируйте ключи на лету, например, с помощью key={Math.random()}. Это приведет к тому, что ключи никогда не будут совпадать между рендерами, что приведет к тому, что все ваши компоненты и DOM будут воссоздаваться каждый раз. Это не только медленно, но и потеряет любой пользовательский ввод внутри элементов списка. Вместо этого используйте стабильный идентификатор на основе данных.

// Обратите внимание, что ваши компоненты не будут получать key в качестве props. Он используется только как подсказка самим React. Если вашему компоненту нужен идентификатор, вы должны передать его как отдельный props: <Profile key={id} userId={id} />.

//# Резюме
/* На этой странице вы узнали:
- Как переместить данные из компонентов в структуры данных, такие как массивы и объекты.
- Как создавать наборы похожих компонентов с помощью JavaScript map().
