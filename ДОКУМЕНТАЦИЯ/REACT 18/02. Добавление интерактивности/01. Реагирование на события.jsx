...
// В отличие от функций рендеринга, обработчики событий не обязательно должны быть чистыми, поэтому это отличное место для того, чтобы что-то изменить — например, изменить значение ввода в ответ на ввод или изменить список в ответ на нажатие кнопки. Однако для того, чтобы изменить некоторую информацию, вам сначала нужно каким-то образом ее сохранить. В React это делается с помощью состояния, памяти компонента. Обо всем этом вы узнаете на следующей странице.

//# Резюме

/*
- Вы можете обрабатывать события, передавая функцию в качестве props такому элементу, как <button>.
- Обработчики событий должны передаваться, а не вызываться! onClick={handleClick}, не onClick={handleClick()}.
- Вы можете определить функцию обработчика событий отдельно или встроенную.
- Обработчики событий определяются внутри компонента, поэтому они могут получить доступ к свойствам.
- Вы можете объявить обработчик событий в родительском элементе и передать его в качестве props дочернему элементу.
- Вы можете определить свои собственные props обработчика событий с именами, специфичными для приложения.
- События распространяются вверх. Вызовите e.stopPropagation()первый аргумент, чтобы предотвратить это.
- События могут иметь нежелательное поведение браузера по умолчанию. Звоните e.preventDefault(), чтобы предотвратить это.
