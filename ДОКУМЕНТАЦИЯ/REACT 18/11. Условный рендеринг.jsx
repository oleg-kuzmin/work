...
// Когда shortcuts мешают писать простой код, попробуйте использовать оператор if и переменную. Вы можете переназначить переменные, определенные с помощью let, поэтому начните с предоставления содержимого по умолчанию, которое вы хотите отобразить, имени:
...
// Если вы не знакомы с JavaScript, такое разнообразие стилей поначалу может показаться ошеломляющим. Однако их изучение поможет вам читать и писать любой код JavaScript, а не только компоненты React! Выберите для начала тот, который вы предпочитаете, а затем снова обратитесь к этому справочнику, если вы забыли, как работают другие.

//# Резюме
/*
- В React вы управляете логикой ветвления с помощью JavaScript.
- Вы можете условно вернуть выражение JSX с помощью инструкции if.
- Вы можете условно сохранить какой-либо JSX в переменную, а затем включить его в другой JSX, используя фигурные скобки.
- В JSX {cond ? <A /> : <B />} означает "если cond - рендерить <A />, иначе <B />".
- В JSX {cond && <A />} означает "если cond - рендерить <A />, иначе ничего".
- Shortcuts распространены, но вам не обязательно их использовать, если вы предпочитаете простые файлы if.
*/
