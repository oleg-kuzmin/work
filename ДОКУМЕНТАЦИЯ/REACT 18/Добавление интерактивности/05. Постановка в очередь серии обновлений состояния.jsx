...
// Давайте попробуем еще один пример. Как вы думаете, какое значение number будет в следующем рендере?
...
1. setNumber(number + 5): numberе сть 0, значит setNumber(0 + 5). React добавляет «заменить на 5» в свою очередь.
2. setNumber(n => n + 1): n => n + 1 это функция обновления. React добавляет эту функцию в свою очередь.
...
// Затем React сохраняет 42 как конечный результат и возвращает его из файла useState.

/* Подводя итог, вот как вы можете думать о том, что вы передаете установщику setNumber:
- Функция обновления (например n => n + 1) добавляется в очередь.
- Любое другое значение (например, число 5) добавляет «заменить на 5» в очередь, игнорируя то, что уже поставлено в очередь.
*/
...
// Аргумент функции обновления обычно называют первыми буквами соответствующей переменной состояния:
...
//# Резюме
/*
- Состояние установки не изменяет переменную в существующей визуализации, но запрашивает новую визуализацию.
- React обрабатывает обновления состояния после завершения работы обработчиков событий. Это называется пакетированием.
- Чтобы обновить некоторое состояние несколько раз в одном событии, вы можете использовать setNumber(n => n + 1)функцию обновления.
*/
