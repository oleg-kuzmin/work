...
// Этот пример иллюстрирует, что компонент может получать разные props с течением времени. props не всегда статичен! Здесь time  меняется каждую секунду, а color меняется, когда вы выбираете другой цвет. Свойства отражают данные компонента в любой момент времени, а не только в начале.

// Однако props неизменны — термин из компьютерной науки, означающий «неизменяемый». Когда компоненту необходимо изменить свои props (например, в ответ на взаимодействие с пользователем или новые данные), ему придется «попросить» родительский компонент передать ему другие props — новый объект! Затем его старые свойства будут отброшены, и в конечном итоге движок JavaScript вернет занятую ими память.

// Не пытайтесь «сменить props». Когда вам нужно отреагировать на пользовательский ввод (например, изменить выбранный цвет), вам нужно будет «установить состояние», о котором вы можете узнать в разделе «Состояние: память компонента».

//# Резюме
/*
- Чтобы передать props, добавьте их в JSX, как и в случае с атрибутами HTML.
- Чтобы прочитать свойства, используйте function Avatar({ person, size }) синтаксис деструктурирования.
- Вы можете указать значение по умолчанию, например size = 100, которое используется для отсутствующих и undefined props.
- Вы можете пересылать все props с помощью <Avatar {...props} />синтаксиса распространения JSX, но не злоупотребляйте им!
- Вложенный JSX в <Card><Avatar/></Card> будет отображаться как props.children компонента Card.
- props доступны только для чтения во времени: каждый рендер получает новую версию props.
- Нельзя менять props. Когда вам нужна интерактивность, вам нужно установить состояние.
*/
