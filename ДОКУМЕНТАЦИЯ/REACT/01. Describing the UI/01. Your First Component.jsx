...
// Поскольку компоненты Profile рендерятся внутри Gallery (даже несколько раз!), мы можем сказать, что Gallery это родительский компонент, отображающий каждый Profile как «дочерний». Это часть магии React: вы можете определить компонент один раз, а затем использовать его в любом количестве мест и столько раз, сколько захотите.
