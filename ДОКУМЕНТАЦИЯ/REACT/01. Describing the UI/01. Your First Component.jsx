...
// Компоненты — это обычные функции JavaScript, поэтому вы можете хранить несколько компонентов в одном файле. Это удобно, когда компоненты относительно небольшие или тесно связаны друг с другом. Если этот файл переполнится, вы всегда можете перейти Profileк отдельному файлу. О том, как это сделать, вы узнаете вскоре на странице об импорте.

// Поскольку Profile компоненты визуализируются внутри Gallery (даже несколько раз!), мы можем сказать, что Gallery это родительский компонент, отображающий каждый Profile как «дочерний». Это часть магии React: вы можете определить компонент один раз, а затем использовать его в любом количестве мест и столько раз, сколько захотите.
