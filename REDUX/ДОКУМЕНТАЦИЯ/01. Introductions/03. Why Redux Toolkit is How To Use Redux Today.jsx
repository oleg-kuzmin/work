...
- По умолчанию JavaScript является изменяемым языком, и для написания неизменяемых обновлений необходимо вручную распространять объекты и обновлять массивы.
...
- Акцент на использовании redux-sagaпромежуточного программного обеспечения как распространенного подхода к написанию побочных эффектов.
- Настаивая на написании типов TS для объектов действий Redux и создании типов объединения, чтобы ограничить, какие действия могут быть отправлены на уровне типа.
*/

// За прошедшие годы мы увидели, как люди на самом деле использовали Redux на практике. Мы видели, как сообщество написало сотни дополнительных библиотек для таких задач, как создание типов действий и создателей, асинхронная логика и побочные эффекты, а также получение данных. Мы также видели проблемы, которые постоянно причиняли боль нашим пользователям, например, случайное изменение состояния, написание десятков строк кода только для одного простого обновления состояния и проблемы с отслеживанием того, как кодовая база согласуется друг с другом. Мы помогли тысячам пользователей, которые пытались изучить и использовать Redux, пытались понять, как все его части сочетаются друг с другом, и были сбиты с толку количеством концепций и объемом дополнительного кода, который им приходилось писать. Мы знаем, с какими проблемами сталкиваются наши пользователи.
...
- Redux Toolkit устраняет случайные мутации, которые всегда были причиной ошибок Redux №1.
- Redux Toolkit устраняет необходимость вручную писать создателей или типы действий.
...
- RTK Query может устранить необходимость написания каких-либо преобразователей, редукторов, создателей действий или перехватчиков эффектов для управления получением данных и отслеживанием состояния загрузки.
*/

// Из-за этого: Мы особенно рекомендуем нашим пользователям использовать Redux Toolkit (@reduxjs/toolkitпакет) и не использовать устаревший redux базовый пакет для любого нового кода Redux сегодня!

// Даже для существующих приложений мы рекомендуем, по крайней мере, отключиться, createStore поскольку configureStore промежуточное программное обеспечение режима разработки также поможет вам обнаружить случайные ошибки мутации и сериализуемости в существующих базах кода. Мы также хотим призвать вас переключиться на редьюсеры, которые вы используете чаще всего (и на те, которые вы напишете в будущем) createSlice — код станет короче и легче для понимания, а улучшения безопасности сэкономят вам время и усилия в будущем.

// Базовый redux пакет все еще работает, но на сегодняшний день мы считаем его устаревшим. Все его API также реэкспортируются из @reduxjs/toolkit, и configureStore он делает все createStore, но с лучшим поведением по умолчанию и улучшенными возможностями настройки.

// Полезно понять концепции нижнего уровня, чтобы лучше понять, что Redux Toolkit делает для вас. Вот почему учебник «Основы Redux» показывает, как работает Redux, без каких-либо абстракций. Но эти примеры показаны исключительно в качестве инструмента обучения, а в конце показано, как Redux Toolkit упрощает старый рукописный код Redux.

// Если вы используете reduxосновной пакет отдельно, ваш код продолжит работать. Но мы настоятельно рекомендуем вам перейти на @reduxjs/toolkit и обновить свой код, чтобы вместо этого использовать API-интерфейсы Redux Toolkit!
