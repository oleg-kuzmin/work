...
RTK включает в себя утилиты, которые помогают упростить многие распространенные случаи использования, включая [настройку хранилища](https://redux-toolkit.js.org/api/configureStore), создание [редюсера](https://redux-toolkit.js.org/api/createreducer) и написание [иммутабельной логики обновления](https://redux-toolkit.js.org/api/createreducer) и даже одновременное создание целых «фрагментов» состояния.
...
// Объекты action по-прежнему передаются в dispatch, но они создаются за нас автоматически.
store.dispatch(incremented()); // {value: 1}
store.dispatch(incremented()); // {value: 2}
store.dispatch(decremented()); // {value: 1}
```

Вместо непосредственного изменения состояния вы указываете мутации, которые должны произойти с простыми объектами, называемыми actions. Затем вы пишете специальную функцию, называемую редюсер, чтобы решить, как каждое действие преобразует состояние всего приложения.
