...
- Компонент должен быть чистым, что означает:
  - Он занимается своим делом. Он не должен изменять какие-либо объекты или переменные, существовавшие до рендеринга.
  - Те же входы, тот же выход. Учитывая одни и те же входные данные, компонент всегда должен возвращать один и тот же JSX.
...
- Вы не должны изменять какие-либо входные данные, которые ваши компоненты используют для рендеринга. Это включает в себя реквизит, состояние и контекст. Чтобы обновить экран, «установите» состояние вместо изменения уже существующих объектов.
- Стремитесь выразить логику вашего компонента в возвращаемом JSX. Когда вам нужно «что-то изменить», вы обычно захотите сделать это в обработчике событий. В крайнем случае вы можете использовать useEffect.
- Написание чистых функций требует некоторой практики, но оно раскрывает мощь парадигмы React.
*/
