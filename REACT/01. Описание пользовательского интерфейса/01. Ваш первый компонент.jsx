...
// Традиционно при создании веб-страниц веб-разработчики размечали их контент, а затем добавляли взаимодействие, добавляя немного JavaScript. Это отлично работало, когда взаимодействие в сети было просто приятным дополнением. Теперь это ожидается по умолчанию для многих сайтов и всех приложений. React ставит интерактивность на первое место, но при этом использует ту же технологию: компонент React — это функция JavaScript, которую вы можете разметить. Вот как это выглядит (вы можете отредактировать пример ниже):
...
// Префикс экспорта по умолчанию — это стандартный синтаксис JavaScript (не специфичный для React). Он позволяет пометить основную функцию в файле, чтобы позже можно было импортировать ее из других файлов. (Подробнее об импорте см. в разделе «Импорт и экспорт компонентов!»).
...
//# ГЛУБОКОЕ ПОГРУЖЕНИЕ
// Компоненты спускаются вниз.

// Ваше приложение React начинается с «корневого» компонента root. Обычно он создается автоматически при запуске нового проекта. Например, если вы используете CodeSandbox или фреймворк Next.js, корневой компонент определяется в Pages/index.js. В этих примерах вы экспортировали корневые компоненты.
