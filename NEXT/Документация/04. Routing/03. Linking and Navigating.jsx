...
//# 3. Partial Rendering
// Частичная отрисовка означает, что только те сегменты маршрута, которые изменяются при повторной отрисовке навигации на клиенте, при этом все общие сегменты сохраняются.

// Например, при навигации между двумя родственными маршрутами, /dashboard/settings и /dashboard/analytics, страницы настроек и аналитики будут отображаться, а макет общей информационной панели будет сохранен.

// Без частичного рендеринга каждая навигация приведет к повторному рендерингу всей страницы на сервере. Рендеринг только того сегмента, который изменяется, сокращает объем передаваемых данных и время выполнения, что приводит к повышению производительности.

//# 4. Soft Navigation
// По умолчанию браузер выполняет жесткую навигацию между страницами. Это означает, что браузер перезагружает страницу и сбрасывает состояние React, такое как перехватчики useState в вашем приложении, и состояние браузера, такое как положение прокрутки пользователя или элемент, находящийся в фокусе. Однако в Next.js App Router использует программную навигацию. Это означает, что React отображает только изменившиеся сегменты, сохраняя при этом состояние React и браузера, без полной перезагрузки страницы.
